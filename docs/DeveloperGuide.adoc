...
The review mechanism is facilitated by the existing `EateryList`.
It extends `EateryList` with a review field, stored internally as an array list of reviews along with other
attributes of an eatery.
Additionally, it implements the following operations:

* `EateryList#review()` -- Adds a review to the eatery identified by the user input.
* `EateryList#editReview()` -- Edits a previously written review.
